...
def isSafe(grid, row, col, num):
    # Check if find the same num in the similar row, return false
    for x in range(9):
        if grid[row][x] == num:
            return False

    # Check if find the same num in the similar column, return false
    for x in range(9):
        if grid[x][col] == num:
            return False

    # Check if find the same num in the 3*3 matrix, return false
    startRow = row - row % 3
    startCol = col - col % 3
    for i in range(3):
        for j in range(3):
